...
class SshGui(tk.Tk):
    ...
    def applyStyles(self) -> None:
        ...
        # Button Styles
        style.configure("Primary.TButton", font=('Segoe UI', 9, 'bold'), padding=Padding.BUTTON)
        style.configure("Secondary.TButton", font=('Segoe UI', 9), padding=Padding.BUTTON)
        style.configure("Success.TButton", font=('Segoe UI', 9, 'bold'), padding=Padding.BUTTON)
        ...
        try:
            if sys.platform.startswith("win"):
                ...
                for term in terminals:
                    if self.isExecutableInPath(term):
                        if term == "gnome-terminal":
                            subprocess.Popen([term, "--", "ssh", host_alias])
                        else:
                            subprocess.Popen([term, "-e", f"ssh {host_alias}"])
                        break
                else:
                    messagebox.showerror("Kein Terminal gefunden", 
                                       "Kein unterstütztes Terminal gefunden.")
                    return
